...
class q_net:
    def __init__(self, name):
        ...
        with tf.variable_scope(name):
            self.obs = tf.placeholder(dtype=tf.float32, shape=[None, 80, 80, 4], name='obs')
            ...
            self.Q = tf.layers.dense(inputs=fc1, units=4, activation=None, kernel_initializer=tf.contrib.layers.xavier_initializer())
